...
def main(args):
  ...
  if not args.save_fully_trained_ref_model:
      fully_trained_model = vgg11(weights=None)
  ...
  with open(f"{args.name_of_experiment}/seed_{args.seed}/fully_trained_reference_model_vgg11_metrics.pkl", "wb") as f:
    pickle.dump((accuracy_list, training_loss_list, testing_loss_list), f)

  with open(f"{args.name_of_experiment}/seed_{args.seed}/vgg11_cka_values_across_epochs.pkl", "wb") as f:
  ...
  with open(f"{args.name_of_experiment}/seed_{args.seed}/vgg11_cka_freeze_configuration.pkl", "wb") as f:
