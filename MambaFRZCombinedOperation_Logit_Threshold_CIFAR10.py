...
def main(args):
  ...
  for epoch in range(args.epochs):
    ...
    for p_index, p in enumerate(conv_active):
      ...
      print("Logits:", pred)
      frz_pred_probs = torch.softmax(
          pred,
          dim=-1
      )
      print("Probabilities after softmax:", frz_pred_probs)
      print("FRZ Prob:", frz_pred_probs[0, 1])
      # prediction_for_freezing = torch.argmax(pred).item()
      if frz_pred_probs[0, 1] >= args.mambafrz_logit_threshold:
